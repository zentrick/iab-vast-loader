...
const passThroughUriReplacer = uri => uri

class VASTLoader extends EventEmitter {
  constructor (uri, options, parent) {
    super()
    this._uri = uri
    this._fetchUriReplacer = passThroughUriReplacer
    if (parent != null) {
  ...
  get fetchUriReplacer () {
    return this._fetchUriReplacer
  }

  set fetchUriReplacer (value) {
    this._fetchUriReplacer = value
  }
  ...
  _fetchUri (uri) {
    const replacedUri = this._fetchUriReplacer(uri)
    const fetching = this._fetchWithCredentials(replacedUri)
    const timingOut = this._createTimeouter(fetching, replacedUri)
    return Promise.race([fetching, timingOut])
      ...
      .catch(err => {
        timingOut.cancel()
        if (this._depth > 1) {
          throw new VASTLoaderError(301, null, replacedUri)
        }
        throw err
      })
  }
